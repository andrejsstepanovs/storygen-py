...
def build_story(suggestion: str) -> Story:
    ...
    story.time_period = TimePeriod(name=tp_names[0], description="") if tp_names else TimePeriod(**available_time_periods[0])
    ...
    loc_chain = FIGURE_LOCATION_PROMPT | llm
    story.location = remove_emojis(loc_chain.invoke({
        "audience": settings.audience,
        "story_json": story.model_dump_json(),
        "general_instruction": GENERAL_INSTRUCTION
    }).content.strip())
    ...
    # 8. Chapters Text
    ch_chain = FIGURE_CHAPTER_PROMPT | llm
    ...
    # 9. Story Title
    title_chain = FIGURE_TITLE_PROMPT | llm
    story.title = remove_emojis(title_chain.invoke({
        "audience": settings.audience,
        "story_json": story.model_dump_json(),
        "general_instruction": GENERAL_INSTRUCTION
    }).content.strip().strip('"'))
